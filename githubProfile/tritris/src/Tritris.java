...
// It would be cool to add configuration for the board and shape dimensions, so one can alter the tiling of the plane
// It should be easier to change the shape types, as in their constituting coordinates
// Restructure Game/Shape?
...
public class Tritris {
	...
	public static void removeFullLayers(LinkedList<Integer> fullLayers, LinkedList<int[]> object) {
		...
		HashMap<TritrisShape, LinkedList<Integer[]>> piecesToRemove = new HashMap<TritrisShape, LinkedList<Integer[]>>();
				...
				for(TritrisShape shape : shapes) {
					...
					for (int[] element : shape.curCords) {
						if(objectCord[0] == element[0] && objectCord[1] + height == element[1]) {
							if(!piecesToRemove.containsKey(shape)){
								piecesToRemove.put(shape, new LinkedList<Integer[]>());
							}
							piecesToRemove.get(shape).add(new Integer[]{element[0], element[1]});
		...
		for(TritrisShape shape : piecesToRemove.keySet()){
			for(Integer[] cord : piecesToRemove.get(shape)){
				shape.removeCord(new int[] {cord[0], cord[1]});
